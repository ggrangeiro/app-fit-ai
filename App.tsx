    ...
    try {
      ...
      if (isVideo && mediaFile.size > 15 * 1024 * 1024) {
         ...
         try {
           ...
           setError("Falha ao otimizar o vídeo. " + (compressError.message || "Tente um arquivo menor."));
      ...
      if (msg.includes("429") || msg.toLowerCase().includes("quota")) {
        setError("Limite de uso excedido (API). Tente novamente em alguns instantes.");
      } else if (msg.includes("safety")) {
        setError("O conteúdo foi bloqueado pelos filtros de segurança da IA. Tente outro vídeo.");
      } else {
        setError("Erro na análise: " + (err.message || "Verifique sua conexão e tente novamente."));
      }
      setStep(AppStep.UPLOAD_VIDEO);
    }
  };
    ...
    <div className="min-h-screen flex flex-col font-[Plus Jakarta Sans]">
      ...
      <main className="flex-grow flex items-center justify-center p-4 md:p-8">
        ...
        {step === AppStep.UPLOAD_VIDEO && (
          <div className="w-full max-w-3xl animate-fade-in">
            <div className="glass-panel rounded-3xl p-6 md:p-12 shadow-2xl">
                ...
                >
                  {mediaPreview ? (
                    mediaFile?.type.startsWith('image/') ? (
                      <img src={mediaPreview} className="h-full w-full object-contain" alt="Preview" />
                    ) : (
                      <video src={mediaPreview} className="h-full w-full object-contain" controls={false} autoPlay muted loop playsInline />
                    )
                  ) : (
                  ...
                  <input id="video-upload" type="file" accept={acceptedFileTypes} className="hidden" onChange={handleFileChange} />
              ...
              {error && (
                <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-200 text-center text-sm flex items-center justify-center gap-2 animate-pulse">
                   ⚠️ {error}
                </div>
              )}
            ...
            {selectedExercise && (
              <div className="glass-panel rounded-2xl p-6 md:p-8 relative overflow-hidden transition-all duration-700 hover:shadow-2xl hover:shadow-blue-900/10 text-left md:text-center">
                 ...
                 <div className="min-h-[60px] md:min-h-[80px] flex items-center justify-start md:justify-center">
                   <p className="text-white text-lg md:text-2xl font-medium leading-relaxed animate-fade-in">
                 ...
                 <div className="flex justify-start md:justify-center gap-2 mt-6">
                    {EXERCISE_TIPS[selectedExercise].map((_, idx) => (
                      <div key={idx} className={`h-1.5 rounded-full transition-all duration-500 ${idx === currentTipIndex ? 'w-8 bg-blue-500' : 'w-2 bg-slate-700'}`} />
                    ))}
                 </div>
              </div>
            )}
        ...
        {step === AppStep.RESULTS && analysisResult && selectedExercise && (
          <ResultView result={analysisResult} exercise={selectedExercise} onReset={resetApp} />
        )}
